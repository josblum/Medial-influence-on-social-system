...
timesteps = 6;                               % Number of Network Simulations
temp_opinion = zeros(network_size,1);        % Initializing a temporaray opinion vector 
k_CF = 1;                                    % Opinion multiplication factor for CF_friends
k_FB = 0.5;                                  % Opinion multiplication factor for FB_friends
...
for t = 1:timesteps
    for xx = 1:network_size     % Iterating through all agents in the network
        ...
        if (agents(xx,3) == 0)          % If agent is normal agent adjust his temp_opinion
            ...
            temp_opinion(xx) = (1-agents(xx,4)) * agents(xx,5) + ...    % His own opinion 
                           agents(xx,4) * ((temp_opinion_CF_hor + temp_opinion_CF_ver) * k_CF + (temp_opinion_FB_hor + temp_opinion_FB_ver) * k_FB) / (agents(xx,1) + agents(xx,2));
        end
    end
