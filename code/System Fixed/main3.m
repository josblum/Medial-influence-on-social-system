...
timesteps = 400;                               % Number of Network Simulations
...
for t = 1:timesteps
    for xx = 1:network_size     % Iterating through all agents in the network
            ...
            if (agents(xx,3) == 0) 
                % His own opinion 
                temp_opinion(xx) = (1-agents(xx,4)) * agents(xx,5) + agents(xx,4) * ((temp_opinionCF_sum * k_CF + temp_opinionFB_sum * k_FB) / (participantsCF*k_CF +participantsFB*k_FB+1));
            end
            
            if (agents(xx,3) == -1)        % If agent is influenced by Specialist or Spreader
                % His own opinion 
                temp_opinion(xx) = (1-agents(xx,4)) * agents(xx,5) + agents(xx,4) * ((temp_opinionCF_sum * k_CF + temp_opinionFB_sum * k_FB + agents(xx,3)*k_IF) / (participantsCF*k_CF + participantsFB*k_FB + k_IF+1));
            end
    end
